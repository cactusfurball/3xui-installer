...
# --- Определение порта панели и открытие его в UFW (постоянно) ---
CONFIG="/etc/x-ui/x-ui.json"
PANEL_PORT="54321"
if [[ -f "$CONFIG" ]]; then
  # если конфиг есть — читаем
...
▶ SSL (самоподписанные, НЕ прописаны в панели — добавьте сами):
   ...
   В панели: Настройки → SSL → укажите пути выше и примените.

▶ Firewall (UFW):
   - SSH (limit)    : ${NEWPORT}/tcp
   - VPN/Xray       : 443/tcp
   - Панель 3x-ui   : ${PANEL_PORT}/tcp
...
▶ Рекомендации по безопасности:
   ...
   4) Настройте VLESS/REALITY на 443 и клиентские маршруты, как в вашем гайде.
